---
title: Markdown
description: Markdown rendering component with syntax highlighting and Mermaid support
order: 4
section: data-display
component: "@duskmoon-dev/el-markdown"
---

import ComponentDemo from '../../../components/ComponentDemo.astro';


# Markdown

The Markdown component renders markdown content with syntax highlighting, GitHub Flavored Markdown support, and optional Mermaid diagram rendering.

## Installation

```bash
npm install @duskmoon-dev/el-markdown
```

For Mermaid diagram support (optional):

```bash
npm install mermaid
```

## Usage

```javascript
import '@duskmoon-dev/el-markdown/register';
```

````html
<el-dm-markdown>
  # Hello World

  This is **bold** and *italic* text.
</el-dm-markdown>
````

## Live Demo

<ComponentDemo title="Markdown Rendering with Front Matter, Color Chips, and Soft Breaks">
  <el-dm-markdown content="---\ntitle: DmMarkdown feature showcase\ntags:\n  - rust\n  - yew\naccent: '#4C86FC'\n---\n# DmMarkdown rendering\n\nDm Markdown renders source text with highlighted front matter and inline color chips.\n\nSoft break, first line\nSoft break, second line\n\n## Inline color chips\n\n| Color | Inline code |\n| --- | --- |\n| Brand blue | `#4C86FC` |\n| White | `#fff` |\n| Black | `#000` |\n| Transparent red | `#FF000080` |" style="width: 100%; max-width: 800px;"></el-dm-markdown>
</ComponentDemo>

Single newlines render as `<br>` elements because the Markdown renderer enables Marked's `breaks: true` option by default.

````html
<el-dm-markdown>
  ---
  title: DmMarkdown feature showcase
  tags:
    - rust
    - yew
  accent: '#4C86FC'
  ---
  # DmMarkdown rendering

  Soft break, first line
  Soft break, second line

  Color previews: `#4C86FC`, `#fff`, `#000`, and `#FF000080`.
</el-dm-markdown>
````

## Properties

| Property | Type | Default | Description |
|----------|------|---------|-------------|
| `content` | `string` | `''` | Markdown content to render (alternative to slot) |
| `sanitize` | `boolean` | `true` | Sanitize HTML in markdown |
| `highlight` | `boolean` | `true` | Enable syntax highlighting |
| `mermaid` | `boolean` | `false` | Enable Mermaid diagram rendering |

## Content Methods

### Using Slot

````html
<el-dm-markdown>
  # Heading

  Content goes here.
</el-dm-markdown>
````

### Using Property

```html
<el-dm-markdown content="# Heading\n\nContent goes here."></el-dm-markdown>
```

### Using JavaScript

```javascript
const md = document.querySelector('el-dm-markdown');
md.content = '# Dynamic Content\n\nUpdated via JavaScript.';
```

## Features

### GitHub Flavored Markdown

The component supports GFM features:

- Tables
- Task lists
- Strikethrough
- Autolinks
- Footnotes
- Soft line breaks by default
- Color previews for HEX, RGB, and HSL values wrapped in backticks
- Leading YAML front matter rendered as a highlighted code block

<ComponentDemo title="GFM Features">
  <el-dm-markdown content="| Feature | Supported |\n|---------|----------|\n| Tables  | ✅        |\n| Tasks   | ✅        |\n\n- [x] Task 1\n- [ ] Task 2\n\n~~Strikethrough text~~" style="width: 100%; max-width: 400px;"></el-dm-markdown>
</ComponentDemo>

````html
<el-dm-markdown>
  | Feature | Supported |
  |---------|-----------|
  | Tables  | ✅        |
  | Tasks   | ✅        |

  - [x] Task 1
  - [ ] Task 2
</el-dm-markdown>
````

### Syntax Highlighting

Code blocks are automatically highlighted:

<ComponentDemo title="Code Syntax Highlighting">
  <el-dm-markdown content="```javascript\nfunction greet(name) {\n  console.log(`Hello, ${name}!`);\n}\n```" style="width: 100%; max-width: 500px;"></el-dm-markdown>
</ComponentDemo>

````html
<el-dm-markdown>
  ```javascript
  function greet(name) {
    console.log(`Hello, ${name}!`);
  }
  ```
</el-dm-markdown>
````

### Mermaid Diagrams

Mermaid diagrams are rendered automatically when the `mermaid` package is installed. Use `no-mermaid` to disable:

<ComponentDemo title="Mermaid Diagram">
  <el-dm-markdown content="```mermaid\ngraph TD\n  A[Start] --> B{Decision}\n  B -->|Yes| C[Process]\n  B -->|No| D[End]\n  C --> D\n```" style="width: 100%; max-width: 500px;"></el-dm-markdown>
</ComponentDemo>

````html
<el-dm-markdown>
  ```mermaid
  graph TD
    A[Start] --> B{Decision}
    B -->|Yes| C[Process]
    B -->|No| D[End]
    C --> D
  ```
</el-dm-markdown>
````

## Styling

The markdown component applies styling through the `.markdown-body` class from `@duskmoon-dev/core`.

### CSS Custom Properties

| Property | Description |
|----------|-------------|
| `--dm-markdown-font-family` | Font family for content |
| `--dm-markdown-line-height` | Line height for paragraphs |
| `--dm-markdown-code-bg` | Background color for code blocks |
| `--dm-markdown-link-color` | Link color |

## Events

| Event | Description |
|-------|-------------|
| `dm-rendered` | Fired after markdown is rendered |
| `dm-error` | Fired if rendering fails |

### Event Handling

```javascript
const md = document.querySelector('el-dm-markdown');
md.addEventListener('dm-rendered', (event) => {
  console.log('Markdown rendered:', event.detail);
});
```

## Security

By default, the component sanitizes HTML to prevent XSS attacks. You can disable this for trusted content:

````html
<el-dm-markdown sanitize="false">
  <div class="custom-html">Custom HTML allowed</div>
</el-dm-markdown>
````

⚠️ **Warning**: Only disable sanitization for trusted content sources.

## Accessibility

- Semantic HTML is generated from markdown
- Headings maintain proper hierarchy
- Images require alt text
- Links are keyboard navigable
- Code blocks have proper language labels
